...
const jwksUrl = 'https://pavan2106.auth0.com/.well-known/jwks.json';
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  try {
    ...
    logger.log('decoded code', JSON.stringify(jwt));
    const res = await Axios.get(jwksUrl);
    logger.log('oauth res', JSON.stringify(res.data));
   ...
   const signingKey = signingKeys.find(key => key.kid === header.kid);
    ...
    return verify(token,  signingKey.publicKey || signingKey.rsaPublicKey, { algorithms: ['RS256'] }) as JwtPayload
  } catch(err){
